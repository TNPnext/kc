...
-(void)updateFrame
{
    ...
    [self.rightButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self).offset(margin - 10);
        make.top.equalTo(self).offset(top);
        make.size.mas_equalTo(CGSizeMake(buttonWidth, buttonHeight));
    }];
...
- (void)wr_setRightButtonWithTitle:(NSString *)title font:(CGFloat)font {
    ...
    [self.rightButton mas_updateConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo([title widthForFont:[UIFont systemFontOfSize:font]] + 10);
    }];
}
...
-(UILabel *)titleLable {
    if (!_titleLable) {
        ...
        _titleLable.font = [UIFont fontWithName:@"STHeitiTC-Light" size:kWRDefaultTitleSize];
